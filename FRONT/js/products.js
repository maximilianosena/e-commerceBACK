...
function showProducts(array) {
  ...
  for (let product of array) {
    let contenido =
      `<div class=container_product onclick="setProductID(${product.product_id})" style="cursor:pointer;">
      <div class=container_animado> <img src="${product.image}" height=300> 
      <span class=texto-hover>${product.product_name} <p>${product.currency} ${product.cost} </p></span> 
...
let url = 'http://localhost:5500/products/'+categories
...
function searching(products) {
  ...
  const filter = products.filter(producto => producto.product_name.toLowerCase().includes(search.value.toLowerCase()) ||
    producto.product_description.toLowerCase().includes(search.value.toLowerCase()));
